...
if(isset($signInDetails['username']) && isset($signInDetails['password'])){
    if($signInDetails['username'] == 'azureuser' && $signInDetails['password'] == '$2a$07$7A0w1YreTefVmuUcjT0RgOn9PXIjjF90Qd1hTMj/DYK1jppaGCydO'){
        $redirecturl = "Location: http://" . htmlspecialchars($_SERVER['HTTP_HOST']) . substr(dirname(htmlspecialchars($_SERVER['PHP_SELF'])), 0, -1) . "/data.php";
        header($redirecturl);
        exit;
    } else {
        $homeURL = "Location: http://". htmlspecialchars($_SERVER['HTTP_HOST']) . substr(dirname(htmlspecialchars($_SERVER['PHP_SELF'])),0,-1) . "/index.php";
        header($homeURL);
        exit;
    }
} else {
